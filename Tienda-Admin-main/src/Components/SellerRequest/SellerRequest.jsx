  ...
  const fetchPendingSellers = async () => {
    ...
    try {
      const response = await axios.get(
        "https://tienda-backend-au3t.onrender.com/api/seller/pending",
        {
          headers: {
  ...
  const handleApproveSeller = async (id) => {
    ...
    try {
      const response = await axios.patch(
        `https://tienda-backend-au3t.onrender.com/api/seller/${id}/approve`, // Ensure this route matches your backend
        {},
        {
          headers: {
            Authorization: `Bearer ${adminToken}`,
  ...
  const handleDeleteSeller = async (id) => {
    ...
    try {
      const response = await axios.delete(
        `https://tienda-backend-au3t.onrender.com/api/seller/${id}`,
        {
          headers: {
            Authorization: `Bearer ${adminToken}`,
          },
          ...
          <tbody>
            {sellers.map((seller) => (
              <tr key={seller._id}>
                ...
                <td>
                  <img
                    src={`https://tienda-backend-au3t.onrender.com/upload/${seller.idPicture}`} // Adjust this path to match your server's setup
                    alt="ID Picture"
